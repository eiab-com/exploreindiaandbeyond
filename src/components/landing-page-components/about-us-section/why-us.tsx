import React from "react";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
import Image from "next/image";
import { whyUsCardData as cardData } from "@/data/content";
const WhyUs = () => {
  return (
    <section
      id="about"
      className="col-span-12 mx-10 h-screen   flex justify-center items-center  relative font-body"
    >
      <div className="left flex flex-col justify-center items-left w-full h-full">
        <p className="font-body text-5xl md:text-6xl  w-full leading-snug mb-10 sm:w-full text-center lg:text-left">
          We specialize in crafting journeys that go beyond typical tourist
          trails.
        </p>
        <div className="card-container w-full flex flex-col flex-wrap justify-center lg:flex-row lg:items-start items-center gap-4 sm:gap-8 sm:justify-center lg:justify-start lg:mt-36">
          {cardData.map((card, index) => (
            <Card
              key={index}
              className="w-full bg-secondary/50 max-w-xs  h-fit hover:border-primary hover:border-2 hover:shadow-2xl relative hover:-translate-y-2 transition-all duration-200"
            >
              <CardHeader className="flex justify-center items-center p-2 sm:p-6 flex-row gap-3">
                <Image
                  src={card.imageSrc}
                  width={50}
                  height={50}
                  alt={card.altText}
                  className="w-12 md:w-12 lg:w-14 h-auto flex"
                />
                <h3 className="text-lg w-full sm:text-xl font-bold">
                  {card.title}
                </h3>
              </CardHeader>
              <CardContent className="p-2">
                <p className="text-sm  lg:inline leading-relaxed">
                  {card.description}
                </p>
              </CardContent>
            </Card>
          ))}
        </div>
      </div>
      <div className="right h-full hidden w-full  overflow-hidden group md:hidden sm:hidden lg:inline-flex">
        <Image
          src="https://images.pexels.com/photos/30743692/pexels-photo-30743692/free-photo-of-adventurous-motorcyclist-at-misty-ladakh-pass.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2"
          alt="Mountains with road"
          width={500}
          height={500}
          className=" w-full h-full object-cover group-hover:scale-[1.1] transition-all duration-500"
        />
      </div>
    </section>
  );
};

export default WhyUs;
